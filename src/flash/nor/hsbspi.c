...
static int hsbspi_alloc_wa(struct flash_bank *bank)
{
	...
	if (hsbspi_algo->buffer_allocated == false) {
		...
		if (retval != ERROR_OK) {
			LOG_ERROR("Unable read current setting of the target clock.");
			return retval;
		}
		...
		if (retval != ERROR_OK) {
			LOG_ERROR("Unable to setup target clock.");
			return retval;
		}
		...
		if (retval != ERROR_OK) {
			LOG_ERROR("Unable to setup target SPI clock rate.");
			return retval;
		}
		...
		if (retval != ERROR_OK) {
			LOG_ERROR("Unable to setup target chip select.");
			return retval;
		}
		...
		if (retval != ERROR_OK) {
			LOG_ERROR("Unable to read target pin share.");
			return retval;
		}
		if (genspi_info->gpio_pin >= 0) {
		...
		if (retval != ERROR_OK) {
			LOG_ERROR("Unable to setup target pin share.");
			return retval;
		}
		...
		if (retval != ERROR_OK) {
			LOG_ERROR("Unable to setup target GPIO outputs.");
			return retval;
		}
		retval = target_write_u32(target,
		...
		if (retval != ERROR_OK) {
			LOG_ERROR("Unable to setup target GPIO inputs.");
			return retval;
		}
	}
	return retval;
